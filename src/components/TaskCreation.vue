  ...
  methods: {
    addTask() {
      if (this.newTask.trim() !== '') {
        const task = {
          id: Date.now(),
          // title: this.newTask, // Corrected to use 'title'
          taskStatus: this.status,
          completed: false,
        };

        this.tasks.push(task);
        this.saveTasksToLocalStorage(); // Corrected line to save 'tasks'
        this.$emit('add-task', task); // Emit the task object
        this.newTask = '';
      }
    },
...
methods: {
  addTask() {
    if (this.newTask.trim() !== '') {
      ...
      // this.saveTasksToLocalStorage(this.tasks); // Save tasks to local storage
